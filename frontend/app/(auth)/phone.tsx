...
export default function PhoneScreen() {
  ...
  const { sendOtp, isLoading, error: storeError, setError } = useAuthStore();
      ...
      >
        <View className="flex-1 px-6 pt-12">
          ...
          {error ? (
            ...
            <Text className="text-sm text-gray-400 mb-4">Мы отправим код через WhatsApp</Text>
          )}
          ...
          <TouchableOpacity
            ...
            className={`rounded-2xl py-4 items-center ${isValidPhone && !isLoading ? 'bg-blue-600' : 'bg-gray-300'}`}
            activeOpacity={0.8}
          >
            <Text
              className={`font-semibold text-base ${isValidPhone && !isLoading ? 'text-white' : 'text-gray-500'}`}
            >

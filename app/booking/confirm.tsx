...
export default function ConfirmScreen() {
  ...
  const { company, date, time, address, options, reset } = useBookingStore();
  const [loading, setLoading] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState<string>('kaspi');

  // Calculate price
  const basePrice = 5000;
  const ecoPrice = options?.ecoFriendly ? 500 : 0;
  const totalPrice = basePrice + ecoPrice;
  ...
  const handleOrder = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      reset(); // Clear booking store after order
    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      ...
      <ScrollView className="flex-1 px-5 pt-6" showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <InfoRow label="Услуга" value="Стандартная уборка" />
          ...
          <InfoRow label="Адрес" value={address ? `${address.street}, ${address.apartment}` : 'Не выбран'} />
        </View>
        ...
        >
          <View className="flex-row justify-between py-1">
            <Text className="text-gray-500">Стандартная уборка</Text>
            <Text className="text-gray-900">{basePrice.toLocaleString()} ₸</Text>
          </View>
          {options?.ecoFriendly && (
            <View className="flex-row justify-between py-1">
              <Text className="text-gray-500">Эко-средства</Text>
              <Text className="text-gray-900">{ecoPrice.toLocaleString()} ₸</Text>
            </View>
          )}
